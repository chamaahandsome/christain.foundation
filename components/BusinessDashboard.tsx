...
export function BusinessDashboard(props: {
  ...
  return (
    <div className="mt-6">
      ...
      {/* Tab row */}
      <div className="flex gap-1.5 overflow-x-auto rounded-xl border border-neutral-200 bg-neutral-50 p-1 dark:border-neutral-800 dark:bg-neutral-900">
        {TABS.map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
      ...
      {tab === "overview" && (
        <div className="mt-8 space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button onClick={() => setTemplateModal(true)} className={newButton}>
              ✍️ Create agreement
            </button>
          </div>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-mono text-xs text-neutral-400"># {c.contractNumber}</p>
                  <p className="mt-0.5 font-semibold">{c.title}</p>
                  {c.clientName && (
                    <p className="text-sm text-neutral-500">{c.clientName}</p>
                  )}
                  {c.preview && (
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                  ...
                  <Link
                    href={
                      c.status === "SIGNED"
                        ? `/verify/${c.id}`
                        : `/studio/channel/${props.channelId}/business/contracts/${c.id}`
                    }
                  ...
                  >
                    {c.status === "SIGNED" ? "View signed ↗" : "Open"}
                  </Link>
                </div>
              </div>
            </div>
      ...
      {creating && (
        <div className="space-y-3 rounded-2xl border border-neutral-200 p-5 dark:border-neutral-800">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} type="email" placeholder="Client email" className={input} />
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="What the quote covers" className={input} />
            <div className="flex items-center gap-1 rounded-lg border border-neutral-300 px-3 dark:border-neutral-700">
              <span className="text-sm text-neutral-500">$</span>
              <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min={1} placeholder="amount" className="w-full bg-transparent py-2 text-sm outline-none" />
            </div>
          </div>
          <button
            disabled={busy || !clientName.trim() || !title.trim() || !amount}
            onClick={() => {
              void call("/api/studio/quotes", "POST", {
                channelId,
                clientName,
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-semibold">{q.title}</p>
      ...
      {creating && (
        <div className="space-y-3 rounded-2xl border border-neutral-200 p-5 dark:border-neutral-800">
          <div className="grid gap-3 sm:grid-cols-2">
            <input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Client name" className={input} />
            <input value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} type="email" placeholder="Client email" className={input} />
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="What the invoice covers" className={input} />
            <div className="flex items-center gap-1 rounded-lg border border-neutral-300 px-3 dark:border-neutral-700">
              <span className="text-sm text-neutral-500">$</span>
              <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min={1} placeholder="amount" className="w-full bg-transparent py-2 text-sm outline-none" />
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-neutral-300 px-3 dark:border-neutral-700">
              <span className="text-sm text-neutral-500">Due</span>
              <input value={due} onChange={(e) => setDue(e.target.value)} type="date" className="w-full bg-transparent py-2 text-sm outline-none" />
            </div>
          </div>
          <button
            disabled={busy || !clientName.trim() || !title.trim() || !amount}
            onClick={() => {
              void call("/api/studio/invoices", "POST", {
                channelId,
                clientName,
                clientEmail,
                title,
                amountCents: Math.round(Number(amount) * 100),
                ...(due ? { dueAt: new Date(`${due}T23:59:59Z`).toISOString() } : {}),
              }).then((ok) => {
                if (ok) {
                  setCreating(false);
                  setClientName("");
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-semibold">{inv.title}</p>
